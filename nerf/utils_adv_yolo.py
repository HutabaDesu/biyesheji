...
from pytorch3d.loss import mesh_laplacian_smoothing, mesh_normal_consistency, mesh_edge_loss, chamfer_distance
...
def random_rotate_mirror_scale(tensor_img, scale_range=(0.8, 1.4)):
    """
    对PyTorch Tensor图片(C,H,W)进行随机旋转、镜像和缩放操作
    参数:
        tensor_img: 输入的图片Tensor,形状为(C,H,W)
        scale_range: 缩放比例范围,默认为(0.8, 1.2)
    返回:
        变换后的图片Tensor
    """
    ...
    # 随机垂直翻转
    if random.random() > 0.5:
        tensor_img = transforms.functional.vflip(tensor_img)
    # 随机旋转(0°, 90°, 180°, 270°)
    angle = random.choice([0, 90, 180, 270])
    tensor_img = transforms.functional.rotate(tensor_img, angle)
    # 缩放后可能需要裁剪或填充以保持原始尺寸
    if scale != 1.0:
        # 方法1: 中心裁剪
...
def generate_eot_adversarial_example(x):
    ...
    epsilon_w = random.uniform(0.2, 0.5)
    epsilon_h = random.uniform(0.2, 0.5)

    pad_width = int(transformed_x.shape[2] * epsilon_w)
    pad_height = int(transformed_x.shape[1] * epsilon_h)
    ...
    transformed_x = torch.nn.functional.pad(transformed_x, (left, right, top, bottom))

    epsilon = random.uniform(0, 0.8)
    brightness_factor = torch.empty(1).uniform_(1 - epsilon, 1 + epsilon)
    contrast_factor = torch.empty(1).uniform_(1 - epsilon, 1 + epsilon)
    saturation_factor = torch.empty(1).uniform_(1 - epsilon, 1 + epsilon)
    # 亮度
    transformed_x = transforms.functional.adjust_brightness(transformed_x, brightness_factor)
    # 对比度
    transformed_x = transforms.functional.adjust_contrast(transformed_x, contrast_factor)
    # 饱和度
    transformed_x = transforms.functional.adjust_saturation(transformed_x, saturation_factor)

    # 旋转
    angle = random.uniform(-90, 90)
    transformed_x = transforms.functional.rotate(transformed_x, angle)

    # 模糊
    kernel_list = [1, 3, 5, 7]
    kernel_size = random.choice(kernel_list)
    transformed_x = transforms.functional.gaussian_blur(transformed_x, kernel_size)
    transformed_x = torch.where(transformed_x == 0, 1, transformed_x)
...
class Trainer(object):
    ...
    def blend_tensor_with_background(self, foreground_tensor, threshold=0.99):
        ...
        参数:
            foreground_tensor: 前景图像Tensor (C×H×W 或 B×C×H×W)
            background_path: 背景图像路径
            threshold: 白色背景判断阈值(0-1)
        返回:
            与原前景Tensor相同格式的融合结果
        """
        # 保存原始Tensor属性
        original_shape = foreground_tensor.shape
        original_device = foreground_tensor.device
        original_dtype = foreground_tensor.dtype
        # 处理batch维度
        has_batch = len(original_shape) == 4
        if has_batch:
            B, C, H, W = original_shape
            foreground_tensor = foreground_tensor.view(-1, C, H, W)  # 合并batch维度
        else:
        ...
        for b in range(output.shape[0]):
            output[b, :, white_mask[b]] = background[0, :, white_mask[b]]
        # 恢复原始形状
        if not has_batch:
    ...
    def train_step(self, data):
        ...
        # MSE loss (stage 0)
        if self.opt.stage == 0:
            outputs = self.model.render(rays_o, rays_d, index=index, staged=False, bg_color=bg_color, perturb=True,
                                        cam_near_far=cam_near_far, shading=shading, **vars(self.opt))
            pred_rgb = outputs['image']
            # print(pred_rgb.shape, gt_rgb.shape)
        ...
        # stage 1
            ...
            # output_all = self.net(image_predicted.unsqueeze(0))
        ...
        if self.opt.stage == 1:
            ...
            if self.opt.lambda_lap > 0:
                loss_lap = laplacian_smooth_loss(self.model.vertices + self.model.vertices_offsets,
                                                 self.model.triangles)
                loss = loss + self.opt.lambda_lap * loss_lap

            if self.opt.lambda_normal > 0:
                if _mesh is None:
                    _mesh = Meshes(verts=[self.model.vertices + self.model.vertices_offsets],
                                   faces=[self.model.triangles])
        ...
        return pred_rgb, gt_rgb, loss, loss_adv, if_attack_success
    ...
    def eval_step(self, data):
        ...
        if self.opt.stage > 0:
            mvp = data['mvp'].squeeze(0)  # [4, 4]
            outputs = self.model.render_eval_stage1(rays_o, rays_d, mvp, H, W, index=index, bg_color=bg_color,
                                                    shading=shading, **vars(self.opt))
        else:
            outputs = self.model.render(rays_o, rays_d, index=index, staged=True, bg_color=bg_color, perturb=False,
                                        cam_near_far=cam_near_far, shading=shading, **vars(self.opt))
    ...
    def test_step(self, data, bg_color=None, perturb=False, shading='full'):
        ...
        if self.opt.stage > 0:
            ...
            outputs = self.model.render(rays_o, rays_d, index=index, staged=True, bg_color=bg_color, perturb=perturb,
                                        cam_near_far=cam_near_far, shading=shading, **vars(self.opt))
    ...
    def train(self, train_loader, valid_loader, max_epochs):
        ...
        for epoch in range(self.epoch + 1, max_epochs + 1):
            ...
            if self.epoch > 150 and self.epoch % 50 == 0 and self.workspace is not None and self.local_rank == 0 and self.opt.stage == 1:
                self.save_checkpoint(full=True, best=False, remove_old=False)

            if (
                    self.epoch % self.save_interval == 0 or self.epoch == max_epochs) and self.workspace is not None and self.local_rank == 0:
    ...
    def test(self, loader, save_path=None, name=None, write_video=True):
        ...
        if write_video:
            ...
            all_preds = np.pad(all_preds, (
                ...
                (0, 0)))
            all_preds_depth = np.pad(all_preds_depth, ((0, 0), (0, 1 if all_preds_depth.shape[1] % 2 != 0 else 0),
                                                       (0, 1 if all_preds_depth.shape[2] % 2 != 0 else 0)))

            imageio.mimwrite(os.path.join(save_path, f'{name}_rgb.mp4'), all_preds, fps=24, quality=8,
                             macro_block_size=1)
            imageio.mimwrite(os.path.join(save_path, f'{name}_depth.mp4'), all_preds_depth, fps=24, quality=8,
                             macro_block_size=1)
    ...
    def test_gui(self, pose, intrinsics, mvp, W, H, bg_color=None, spp=1, downscale=1, shading='full'):
        ...
        if downscale != 1:
            preds = F.interpolate(preds.unsqueeze(0).permute(0, 3, 1, 2), size=(H, W), mode='nearest').permute(0, 2, 3,
                                                                                                               1).squeeze(
                0).contiguous()
            preds_depth = F.interpolate(preds_depth.unsqueeze(0).unsqueeze(1), size=(H, W), mode='nearest').squeeze(
                0).squeeze(1)
    ...
    def train_one_epoch(self, loader):
        ...
        for data in loader:
            ...
            preds, truths, loss_net, loss_adv, if_attack_success = self.train_step(data)
            total_adv_loss += loss_adv
            num_attack_success += if_attack_success
            self.optimizer.zero_grad()
            ...
            if self.opt.stage == 1 and self.opt.refine and self.global_step in self.opt.refine_steps:
                ...
                self.optimizer = self.optimizer_fn(self.model)
                self.lr_scheduler = self.lr_scheduler_fn(self.optimizer)
        print('\nadv loss is {}'.format(total_adv_loss / num_data))
        print('\nNow ASR is: {}'.format(num_attack_success / num_data))
        if self.ema is not None:
            self.ema.update()
            ...
            for data in loader:
                ...
                if self.local_rank == 0:
                    ...
                    save_path_error = os.path.join(self.workspace, 'validation',
                                                   f'{name}_{self.local_step:04d}_error_{metric_vals[0]:.2f}.png')  # metric_vals[0] should be the PSNR
