...
from .utils_adv import custom_meshgrid, plot_pointcloud, safe_normalize
...
class NeRFRenderer(nn.Module):
    ...
    def get_params(self, lr):
        ...
        if self.individual_codes is not None:
            params.append({'params': self.individual_codes, 'lr': self.opt.lr, 'weight_decay': 0})
    ...
    def export_stage1(self, path, h0=2048, w0=2048, png_compression_level=3):
        ...
        def _export_obj(v, f, h0, w0, ssaa=1, cas=0):
            ...
            xyzs = xyzs.view(-1, 3)
            mask = (mask > 0).view(-1)
            print('xyzs_shape is {}'.format(xyzs.shape))
            if self.opt.contract:
                xyzs = contract(xyzs)
        ...
        print('cascade_number is:{}'.format(self.cascade))
        for cas in range(self.cascade):
            cur_v = v[self.v_cumsum[cas]:self.v_cumsum[cas+1]]
            cur_f = f[self.f_cumsum[cas]:self.f_cumsum[cas+1]] - self.v_cumsum[cas]
    ...
    # phase 2
    def render_stage1(self, rays_o, rays_d, ifrandom, mvp, h0, w0, index=None, bg_color=None, shading='full', **kwargs):
        ...
        if mask_flatten.any() and not ifrandom:
            ...
            rgbs = self.rgb(xyzs, dirs, ind_code, shading)[0]
      ...
      # phase 2
    def render_eval_stage1(self, rays_o, rays_d, mvp, h0, w0, index=None, bg_color=None, shading='full', **kwargs):
