...
def rand_poses(size, device, radius=1, theta_range=[60, 120], phi_range=[-60, 60], gamma_range=[-60, 60]):
    ...
    x = np.random.uniform(-2, 2, size)  # x in range [-1, 1]
    y = np.random.uniform(-2, 2, size)  # y in range [-1, 1]

    poses = []
    for i in range(size):
        all_args = [gamma[i], theta[i], phi[i], radius[i] * random.uniform(4, 10), x[i], y[i]]
